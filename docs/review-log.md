...
## #14 2026-07-07 — 軽量モード（変更有無チェック）

- **修正キュー**: なし（#13 は INFO のみ）
- **変更**: #13 のコミット（`6cd58cb`）以降、リポジトリへの変更ゼロ。CI green・fmt ドリフトなし
- **判定**: レビュー対象となる差分が存在しないため観点レビューはスキップ。2 回連続で実質的な新規指摘ゼロ
- **次回**: 変更がなければ同様に軽量モード。頻度見直し 3 案（停止 / push トリガー型 / 間引き）はユーザー判断待ちのまま

---

## #13 2026-07-07 — 観点: セキュリティ（4 巡目・アタッチ配線監査）

### 今回修正したこと（前回指摘の解消、コミット `3990b85`）

- **12-2 修正済み (HIGH)**: `aws_workspaces_directory.workspace_creation_properties` に `custom_security_group_id` を配線し、stack から `sg_workspaces_id` を接続。**AD 13 ポートの SG ルール一式がようやく WorkSpaces インスタンスに効く状態になった**
- 教訓の恒久化: CLAUDE.md 連動ルールに「**定義 ≠ 有効** — アタッチ/配線まで書けて完了」を追加（#8-1・#12-2 の再発防止）
- 注記: ローカル make check がネットワーク要因でタイムアウトしたため、変更ユニット単体 validate + CI で全体検証

### 確認事項（12-2 の教訓を全リソースへ適用）

- 全 SG のアタッチ先の実在
- KMS キー・SNS トピック・DLQ・インスタンスプロファイル・IAM ロールの配線

### 気づいた点

| # | 深刻度 | 場所 | 内容 |
|---|---|---|---|
| 13-1 | INFO | 配線監査結果 | **全リソース配線済みを確認** — SG 3 種（endpoints/workspaces/image_builder）・KMS 2 鍵・SNS・DLQ・インスタンスプロファイル・Lambda ロールすべてにアタッチ先あり。saml ロールのみ TF 外（Entra ID 側設定）だがコメントで明示済み |

### 収束判定（再）→ ループ頻度の見直し提案を再提示

12-2 の修正・全面配線監査の完了により、静的レビューの観点 4 種 × 4 巡 + 配線監査が出揃った。残タスクは #12 時点と同じ「このループでは進められない 3 件」（9-1 設計判断待ち / 8-2 実機検証待ち / 1-7 保留）。**#12 で保留した頻度見直し提案を再提示する**: (a) push トリガー型へ切替え (b) 2〜3 時間間隔へ間引き (c) ループ停止して引き継ぎ。

---

## #12 2026-07-06 — 観点: 問題の有無（4 巡目・収束判定）

### 今回修正したこと（前回指摘の解消、コミット `a8f070a`）

- **11-1 修正済み**: architecture.md ネットワーク表を AD フルポートセットの記述に更新
- **11-2 修正済み**: README の「Windows Update 焼き込み」に要検証注記（9-1 未解決の間、確定事実のように読ませない）

### 確認事項

- 静的整合の全面再チェック（outputs / 変数供給 / stack_vars 消費 / CI）
- 収束判定（2 回連続 LOW 以下なら頻度間引きを提案するルール）
...
| 残タスク | 必要なもの |
|---|---|
| 9-1 update-windows 到達性の方式選択 | ユーザーの設計判断（NAT+FQDN / WSUS / ベース AMI 依存） |
| 8-2 ディレクトリ登録型の確認 | AWS 認証環境での plan / 実機検証 |
| 1-7 VPC エンドポイントポリシー | 価値低・保留妥当 |

**提案**: 30 分間隔の継続はコスト過剰。(a) push トリガー型（変更があったときのみレビュー）へ切替え、(b) 2〜3 時間間隔へ間引き、(c) ループ停止して判断待ち 2 件をユーザーへ引き継ぎ — のいずれかをユーザーが選択するまで、次回以降は**新規変更の有無チェックのみの軽量モード**で回す。

---

## #11 2026-07-06 — 観点: ドキュメントの不完全さ（3 巡目）

### 今回修正したこと（前回指摘の解消、コミット `78366ec`）

- **10-1 修正済み**: vpc の `region` 変数を削除し `data.aws_region` から導出。stack input / stack_vars からも削除（root.hcl が単一情報源に）
- **10-2 修正済み**: stack_vars の未参照 `environment` を削除（Environment タグは root.hcl の default_tags が単一情報源）

### 確認事項

- #10 の AD ポートセット追加が architecture.md に反映されているか
- 9-1（設計判断待ち）の判断材料への導線
- README の記述と 9-1 未解決状態の整合

### 気づいた点（未修正 → 次回対応）

| # | 深刻度 | 場所 | 内容 |
|---|---|---|---|
| 11-1 | LOW | `architecture.md:112` | ネットワーク表の SG 行が「WorkSpaces → AD は LDAP/LDAPS/DNS のみ」のまま — #10 で追加した AD フルポートセット（Kerberos/RPC/SMB 等 13 種）が未反映 |
| 11-2 | LOW | `README.md:22` | image-builder の説明「Windows Update 焼き込み」は 9-1（閉鎖網で Update 到達不能の疑い）が未解決のため、現状では**動作未確認の機能を確定事実のように読ませる**。「(要検証: 閉鎖網での到達性 — 引き継ぎ表参照)」の注記を付けるべき |
| 11-3 | INFO | 導線確認結果 | 9-1 の判断材料は引き継ぎ表 → review-log #9-1 で辿れる。セッションポリシー表・ユニット一覧・依存グラフは現行コードと一致 |

### 次回の確認事項

1. **修正**: 11-1（ネットワーク表更新）・11-2（README 注記）
2. **保留継続**: 9-1（設計判断待ち）・8-2（実機検証待ち）・1-7
3. **新規レビュー観点**: 問題の有無（4 巡目）— 全体の静的整合の定期再確認。指摘が枯れてきているため、2 回連続で新規指摘が LOW 以下のみなら**ループ頻度の間引き（30 分 → 2〜3 時間 or 変更トリガー型）をユーザーに提案**する
